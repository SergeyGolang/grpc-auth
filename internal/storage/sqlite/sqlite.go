...
// New creates a new instanse of the SQLite storage
func New(storagePath string) (*Storage, error) {
	const op = "storage.sqlite.New"

	// Передаём путь storage
...
func (s *Storage) SaveUser(ctx context.Context, email string, passHash []byte) (int64, error) {
	...
	// Забористая проверка на уникальность, поле email в db должно быть уникальным
	if err != nil {
		var sqliteErr sqlite3.Error
		// Конвертируем тип ошибки и проверяем её
	...
	// Получем ID последней созданной записи
